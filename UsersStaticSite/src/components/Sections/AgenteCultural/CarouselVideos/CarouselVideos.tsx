...
function CarouselVideos({ videos, isLoading }: Props): React.ReactNode {
  ...
  return (
    <div className="CarouselVideos">
      <section className="CarouselVideos__content mt-[50px] md:mt-[80px] w-full flex flex-col flex-grow items-start md:px-0 md:max-w-[770px] md:justify-between lg:max-w-[944px] lg:px-0 xl:max-w-[540px]">
        {isLoading ? (
          <CarouselSkeleton />
        ) : (
          <>
            <h2 className="CarouselVideos__title text-h2 mb-4 font-bold text-[24px] leading-[1.4] mb-[16px]">Vídeos</h2>
            <article className="CarouselVideos__items flex w-full justify-start overflow-x-auto overflow-y-hidden snap-x snap-mandatory touch-pan no-scrollbar gap-2 md:flex-wrap md:snap-none lg:gap-2">
              {videos?.map((video, index) => (
                <div
                  tabIndex={0}
                  className="CarouselVideos__item group relative w-[171.33px] h-[126px] flex-shrink-0 cursor-pointer overflow-hidden rounded-xl"
                  key={index}
                  onClick={() => {
                    setIsModalOpen(true);
                ...
                >
                  <div className="CarouselVideos__item-video absolute inset-0 transform group-hover:scale-110 transition-all duration-250">
                    <ReactPlayer
                      url={video.videoUrl}
                      controls={false}
                      width="100%"
        ...
        {isModalOpen && (
          <div className="app-container">
            ...
            >
              ...
              <div className="flex w-full h-[90px] items-center justify-center overflow-x-auto overflow-y-hidden snap-x snap-mandatory touch-pan no-scrollbar gap-2 pl-4 md:justify-center md:pl-0 md:max-w-[770px] lg:max-w-[944px] lg:h-[120px] xl:max-w-[1140px] xl:h-[140px]">
                {videos?.map((video, index) => (
                  <div
                    key={index}
