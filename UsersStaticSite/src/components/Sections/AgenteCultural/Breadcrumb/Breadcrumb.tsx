    ...
    <div className="Breadcrumb">
      <div className="app-container">
        <nav
          ...
          className="Breandcrumb__content flex items-center justify-start mt-4 w-full h-8 pl-[20px] md:pl-[4px]"
        >
          <ol className="flex items-center justify-center gap-1 flex-wrap">
            <li className="flex items-center">
              <Link to="/" className="flex items-center">
                <span className="text-level-2 underline underline-offset-2 font-medium leading-[1.7] text-gray-500 hover:text-primary transition-all duration-300 dark:text-white dark:hover:text-secondary">
                  Home
                </span>
              </Link>
            </li>
            ...
            {breadcrumbItems.map((item, index) => (
              ...
              >
                      ...
                      <ul className="p-2">
                        {breadcrumbItems.slice(0, 1).map((pathItem, i) => (
                          <li key={i}>
                            <Link
                              to={pathItem.path}
                              className="capitalize text-level-2 leading-[1.7] bg-primary hover:bg-primaryDark dark:bg-secondary dark:hover:bg-secondaryDark rounded-md text-white dark:text-black block py-1 px-2"
                            >
                ...
                {index === breadcrumbItems.length - 1 && (
                  <span
                    className="text-level-2 leading-[1.7] text-primary dark:text-gray-400 font-medium"
                    aria-current="page"
                  >
                    {title}
                  </span>
